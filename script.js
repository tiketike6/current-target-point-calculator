(() => {
    ...
    const calculateTargetPoint = () => {
        ...
        const endOfTodayUnix = dayjs().endOf('d').unix();
        const nowUnix = dayjs().unix();

        $('#label-today').text(`${dayjs().format('M/D')}の目標pt`);
        $('#target-today').text(
            Math.round((targetEnd * (endOfTodayUnix - datetimeStartUnix)) / (datetimeEndUnix - datetimeStartUnix)).toLocaleString()
        );
        $('#label-now').text(`${dayjs().format('M/D H:mm')}の目標pt`);
        $('#target-now').text(Math.round((targetEnd * (nowUnix - datetimeStartUnix)) / (datetimeEndUnix - datetimeStartUnix)).toLocaleString());
